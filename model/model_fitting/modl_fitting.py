...
def ordlogit(df):
    ...
    # Makes R recognise counts_truth_percent as an ordered factor. 
    ro.r("rdf$counts_truth_percent = factor(rdf$counts_truth_percent, ordered = T)")    

    # Fits an ordinal logistic regression in R. 
    ro.r("fit <- lrm(counts_truth_percent ~ counts_associated, data=rdf, x=TRUE, y=TRUE)")
    print(ro.r("fit"))

    # Cross validates the model and extracts the accuracy score. 
    ro.r("crossval <- validate(fit, method='cross', B=2, bw=TRUE)")
    ro.r("print(crossval[1])")
    
    # Formats the accuracy score for Python. 
    ascore_raw = ro.r("crossval[1]")
    ascore = "" 
    for c in str(ascore_raw):
        if str(c).isdigit() or c == ".":
            ascore+=str(c)

    return [0, 0, float(ascore), query_time]
